...
class Dashboard2:
    ...
    def saveNews(self, news):
      user_id = self.user  # Assuming self.user is a single integer representing the user ID
    ...
    def display_saved_news(self):
     ...
     saved_news_data = database.getSavedNews((self.user,))
    
    # Check if `saved_news_data` is a list (or iterable) before proceeding
     if not isinstance(saved_news_data, list):
        messagebox.showinfo("No Saved News", "There are no saved news articles to display.")
        return
